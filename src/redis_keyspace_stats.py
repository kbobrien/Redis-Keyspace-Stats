...
parser = argparse.ArgumentParser(description='Scan all keys in redis and analyse first X chars.')
parser.add_argument('-c', dest='char_size', action='store', type=int, help='Number of characters to prefix', default=3)
...
parser.add_argument('-t', dest='test', action='store_true', help='Performs a test run on first 10,000 keys.', default=False)
parser.add_argument('-d', dest='delimiter', action='store', type=str, help='Keyspace delimeter used', default=':')
parser.add_argument('-m', dest='match', action='store', help='Match pattern to use for scaning keyspace', default='*')
parser.add_argument('-D', dest='db', action='store', type=int, help='Redis database to use', default=0)
...
print('-------')
print('{:<20}{:>10}{:>15}{:>25}{:>20}{:>35}{:>20}{:>20}'.format('Prefix', 'Type', 'NoKeys', 'TotalBytes', 'AvgBytesPerKey', 'TotalItems', 'AvgItemsPerKey', 'AvgBytesPerItem'))
